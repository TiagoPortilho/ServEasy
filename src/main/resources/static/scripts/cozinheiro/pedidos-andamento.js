    ...
    fetch('/api/orders?status=EM_ANDAMENTO')
        ...
        .then(data => {
            ...
            if (data.status === 'success' && data.data) {
                displayPedidosAndamento(data.data);
            } else {
...
function displayPedidosAndamento(orders) {
    ...
    container.innerHTML = orders.map(order => {
        ...
        return `<div class="pedido-card">
            ...
            <div class="pedido-tempo">
                ...
                <span>Em preparo há ${timeDisplay} minutos</span>
            </div>
            
            <div class="pedido-acoes">
                <button onclick="finalizarPreparo(${order.id})" class="btn btn-success">
                    <i class="fas fa-check"></i> Finalizar Preparo
                </button>
            </div>
        </div>`;
    }).join('');
...
function finalizarPreparo(orderId) {
    ...
    .then(data => {
        if (data.status === 'success') {
            alert('Preparo finalizado!');
            loadPedidosAndamento();
        } else {
            alert('Erro ao finalizar preparo: ' + data.message);
        }
    })
    .catch(error => {
        console.error('Erro:', error);
        alert('Erro ao finalizar preparo');
    });
}

...
public class CardapioDono extends javax.swing.JFrame {
    ...
    private int IdValorSelecionado() {
        ...
        if (row == -1) {
            JOptionPane.showMessageDialog(this, "Por favor, selecione um prato na tabela.", "Aviso", JOptionPane.WARNING_MESSAGE);
            return -1;
        }
        return (int) tblCardapio.getValueAt(row, 0);
    }
    ...
    private void btnRemovPratoActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnRemovPratoActionPerformed
        int id = IdValorSelecionado();
        if (id == -1) {
            return;
        }
        int confirmar = JOptionPane.showConfirmDialog(this,
                "Tem certeza que deseja deletar o prato selecionado?",
                "Confirmar Deleção", JOptionPane.YES_NO_OPTION);

        if (confirmar == JOptionPane.YES_OPTION) {
            try {
                new PratoDAO().delPrato(id);
                atualizarTabela();
            } catch (SQLException ex) {
                new TelaErro(ex.getMessage()).setVisible(true);
            }
